 ...
 * 
 * The LoaderArray should always be the object that is used by other Loader implementations.
 ...
 * their origin/from state or their destination/to state) then at least make sure 
 * that those states share the exact same data.
...
class LoaderArray implements Loader {
    ...
    /**
     * @param Transition[] $transitions the transitions to be loaded
     */
        ...
        foreach($transitions as $transition) {
        	if(!is_a($transition, 'izzum\statemachine\Transition')){
                throw new Exception('Expected Transition (or a subclass), found something else: ' . 
                        get_class($transition), Exception::BAD_LOADERDATA);
            }
        	$this->add($transition);
        }
    }
