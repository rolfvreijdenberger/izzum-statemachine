 ...
 * example:
     ...
     * State C
     * transition A->C (rule: can always take place to 'force' a transition from A to B
     ...
     * 
     * Now when we use a cronjob where we loop over items the code would be like this:
     * //pseudocode
 ...
 * statemachine. It is not a problem when we use the 'apply' method on the sm
 * since we provide a transition name to that method, forcing the sm to execute
 * only that transition if it can. But keep in mind that there are situations 
 * where you would just want the sm to decide what it does by calling 'run'
 ...
 * 
 * for exmample: \a cron job would not set data and the rule for transition A->B would
 ...
 * example: 
     ...
     *     $sm->apply(Utils::getTransitionName(Data::STATE_A, Data::STATE_C));
     *     
