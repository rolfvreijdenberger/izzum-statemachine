 ...
 * model) for a Context object (by using the associated Identity object of the Context)
 ...
 * with a subclass for your application domain if necessary (and the configuration can be done
 ...
 * It also allows us to use the Identifier object for our rules and commands, so you
 ...
 * which would be used for a statemachine that uses rules and commands to act upon an 'Order' object 
...
class EntityBuilder {
     ...
     * @param boolean $create_fresh_entity optional. if true, then a new instance is always created,
     * 		else it might be cached if used for the same Identifier (performance).
     * 		Since a statemachine might run multiple transitions in memory and alter the data in the persistence layer, 
     * 		there might be a need to refresh the entity (create it again) if the in memory object and the data
     * 		in the persistence layer are not synchronized. this might cause rules or commands to act on 
    ...
    public final function getEntity(Identifier $identifier, $create_fresh_entity = false)
    {
        try {
            ...
            //a new reference to the entity when the builder is used on a new Identifier.
    ...
    protected function build(Identifier $identifier)
    {
        ...
        //that can be used in your rules and commands to build your domain logic or build
        //your domain object. specialized builders return a domain
        //model that will be accepted by your specialized rules/commands for your application.
        return $identifier;
    }
